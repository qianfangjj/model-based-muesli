...
class SummaryTools(object):
  ...
  def print_summary(self, games):
    ...
    game_num = 0
    for game in games:
      print(f"\ngame_num: {game_num}, game_reward_sum: {sum(game.history.rewards)}")
      game_num +=1
...
class Evaluator(SummaryTools):
  ...
  def play_game_muesli(self, environment):
    assert self.network is not None, ".load_network() needs to be called before playing."
    ...
    while not game.terminal:
      ...
      #     if actions_applied == self.config.apply_mcts_actions:
      #       break
      action, logits_results = self.config.select_action_with_logits(initial_inference.policy_logits[0],
                                                                     game.action_space, self.config.temperature)
      game.history.root_values.append(initial_inference.value.item())
      game.pred_values.append(initial_inference.value.item())
      # game.store_search_statistics(root)
      game.apply(action)
    ...
    # print(msg.format(game.step, np.sum(game.history.rewards), np.sum(game.pred_rewards),
    #                  np.mean(game.pred_values), np.mean(game.history.root_values)))
    msg += "length: {:.1f}, return: {:.1f},  pred value: {:.1f}"
    print(msg.format(game.step, np.sum(game.history.rewards),
                     np.mean(game.pred_values)))
...
def run(evaluator, seed=None):
  ...
  with torch.inference_mode():
    ...
    game = evaluator.play_game_muesli(environment)
