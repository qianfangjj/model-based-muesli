...
class PrioritizedReplay():
  ...
  def sample_batch(self):
    ...
    batch_actions = []
    idxs = []
    batch_observations = np.zeros((self.batch_size, self.target_length, *self.obs_space), dtype=np.float32)
  ...
  def insert_target(self, batch_idx, history, step, target_rewards,
    ...
    for idx, current_index in enumerate(range(step, step + self.num_unroll_steps + 1)):
      ...
      if current_index < end_index:
        ...
        target_policies[batch_idx, idx, :] = history.policy_logits[current_index]
        target_rewards[batch_idx, idx] = last_reward
        target_values[batch_idx, idx] = value
        batch_observations[batch_idx, idx, :] = history.observations[current_index]
      else:
        target_policies[batch_idx, idx, :] = self.absorbing_policy
        target_rewards[batch_idx, idx] = last_reward
        target_values[batch_idx, idx] = 0
        batch_observations[batch_idx, idx, :] = history.observations[step]
