...
class Config(object):
  ...
  @staticmethod
  def select_action_with_logits(logits, action_space,temperature=0.):
    actions = [action for action in list(action_space)]
    sp = torch.softmax(logits, dim=0)
    sp = np.asarray(sp).astype('float64')
    sp = sp / sum(sp)
    logits_result = sp.tolist()
    if temperature:
      idx = np.random.choice(len(actions), p=sp)
    else:
      idx = np.argmax(sp)
      # idx = np.random.choice(np.where(visit_counts == visit_counts.max())[0])
    action = actions[idx]
    return action, logits_result
...
def make_config():
  ...
  ### Muesli
  muesli = parser.add_argument_group('muesli')
  muesli.add_argument('--threshold_c', type=float, default=1.0)
  muesli.add_argument('--simulation_depth', type=int, default=1)
  muesli.add_argument('--clip_param', type=float, default=0.1)
  muesli.add_argument('--num_sampled_actions', type=int, default=16)
  muesli.add_argument('--alpha_target', type=float, default=0.1)
...
def get_evaluation_args():
  ...
  parser.add_argument('--saves_dir', nargs='+', type=str, default=['runs/Breakout-ramNoFrameskip-v4/saves'])
  parser.add_argument('--nets', nargs='+', type=str, default=['45700'])
